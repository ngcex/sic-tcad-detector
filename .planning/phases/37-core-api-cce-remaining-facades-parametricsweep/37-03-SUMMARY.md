...
provides:
  ...
  - "ParametricSweep re-exported from etna (from etna import ParametricSweep)"
...
key-decisions:
  ...
  - "DeviceConfig/SimResult imported only under TYPE_CHECKING (mirrors etna.api.results) to avoid an import cycle once etna/__init__.py re-exports these names"
...
- **`ParametricSweep`** added in a new `etna/api/sweep.py` as an `@dataclass` with fields `base_config`, `param: str`, `values: list`, `sim_fn: Callable`, `sim_kwargs: dict = field(default_factory=dict)`. `run()` loops over `values`, clones the config with `replace(self.base_config, **{self.param: value})`, calls `self.sim_fn(cfg_i, **self.sim_kwargs)`, and returns the collected `list[SimResult]`. Implemented exactly per design spec section 3.5.
...
- **Import-cycle avoidance** — `DeviceConfig` and `SimResult` are imported only under `if TYPE_CHECKING:` (mirroring `etna.api.results`), so `etna/__init__.py` re-exporting these names alongside `ParametricSweep` does not create a runtime cycle.
- **Re-export** — `from etna.api.sweep import ParametricSweep` added to `etna/__init__.py` and `"ParametricSweep"` appended to `__all__`.
...
- `etna/api/sweep.py` — new. `ParametricSweep` `@dataclass` + `run()`. Uses `from __future__ import annotations`, `from dataclasses import dataclass, field, replace`, `from typing import TYPE_CHECKING, Callable`; `DeviceConfig`/`SimResult` under `TYPE_CHECKING`. Class docstring shows the spec-3.5 usage and documents the `dataclasses.replace`-vs-`setattr`/`eval` security choice.
- `tests/test_api_sweep.py` — new. Module-level `fake_sim` + 3 fast unit tests; imports `numpy`, `pytest`, `from etna import DeviceConfig, ParametricSweep`, `from etna.api.results import SimResult`. No devsim import or facade call.
- `etna/__init__.py` — added the `ParametricSweep` import line and `__all__` entry.
...
- Kept the module runtime-import-free of `DeviceConfig`/`SimResult` (`TYPE_CHECKING`-only) to preserve the same cycle-avoidance discipline `etna.api.results` already uses.
...
- LIB-07 satisfied: `from etna import ParametricSweep` imports; `.run()` over 2+ values returns a `list[SimResult]` of correct length with the swept field injected into each config.
- The public `etna` API is now complete for this phase: 9 facades (run_cv/run_field/run_cce + 6 remaining) plus `ParametricSweep` to iterate any of them.

## Self-Check: PASSED

- `etna/api/sweep.py` — FOUND
- `tests/test_api_sweep.py` — FOUND (defines `fake_sim`; 3 tests; grep `import devsim|devsim\.` == 0)
- `etna/__init__.py` `ParametricSweep` re-export — FOUND (in import block + `__all__`)
- `grep -Ec "replace\(.*self\.param" etna/api/sweep.py` == 1; `setattr(` == 0; `eval(` == 0
