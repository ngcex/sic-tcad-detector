...
verified: 2026-06-16T09:07:24Z
status: gaps_found
score: 3/4 must-haves verified
gaps:
  - truth: "User can compute a 2D C-V curve over the full bias range and observe agreement with the validated 1D C-V (R² >= 0.99) at the device center — CONS-01 SC#2"
    status: partial
    reason: "TestCalibrationCV::test_2d_vs_1d_cv_centerline uses voltages [0.0, -2.5, -5.0, -7.5, -10.0] (max -10 V) instead of the full clinical range [0, -5, -10, -15, -20, -30, -40, -50 V] required by the plan and CONS-01 SC#2. The ROADMAP SC#2 reads 'full bias range'; the plan text is explicit: 'sampled at 0, -5, -10, -15, -20, -30, -40, -50 V'. The test passes in 17 s at low bias but does NOT cover -15 V through -50 V."
    artifacts:
      - path: "tests/test_device2d.py"
        issue: "TestCalibrationCV.test_2d_vs_1d_cv_centerline: voltages list stops at -10 V; missing -15, -20, -30, -40, -50 V checkpoints required by CONS-01 SC#2"
    missing:
      - "Extend the voltages list in test_2d_vs_1d_cv_centerline to [0.0, -5.0, -10.0, -15.0, -20.0, -30.0, -40.0, -50.0] (the full clinical range). The calibrated defaults already converge to -50 V (TestReverseBiasConvergence passes), so the solver can handle the sweep; the test just needs the wider voltage list."
---
...
| 2   | User can compute 2D C-V over the FULL bias range and observe R² >= 0.99 agreement with 1D C-V at device center (CONS-01 SC#2)                                   | PARTIAL  | TestCalibrationCV passes in 16.7 s but only sweeps [0, -2.5, -5, -7.5, -10 V] — max -10 V instead of the plan-required [0, -5, -10, -15, -20, -30, -40, -50 V]. The R² assertion exists and passes at low bias but the full-range clinical criterion is NOT covered.                                                                                                  |
...
**Score:** 3/4 truths verified (1 partial — SC#2 voltage range)

### Deferred Items

None. The gap is not addressed in any later phase.
...
| `.planning/phases/26-graded-doping-2d-calibration/26-REGRESSION-REPORT.md`  | Per-notebook PASS/FAIL report                                                  | VERIFIED (with caveat) | Shows 15/20 PASS, 5 TIMEOUT, 0 functional regressions. `overall: PASS # no functional regression...` (inline comment appended; plan acceptance criterion `grep -E "^overall: PASS$"` technically fails due to comment, but the verdict is honestly documented).                         |
...
| `tests/test_device2d.py`                                                    | TestReverseBiasConvergence (6 methods, no xfail) + TestCalibrationCV (partial) | PARTIAL                | TestReverseBiasConvergence: xfail removed, 6 methods, real assertions, `reset_devsim_fully` teardown. TestCalibrationCV: xfail removed, R² assertion present — but voltage range only to -10 V (see gap).                                                                               |
...
| ------------------------------------------- | ---------------------------------- | ------------------------------------------------------- | -------------------------------------- | ----------------------------------------- |
| `tests/test_device2d.py::TestCalibrationCV` | `W_2d` depletion widths            | `extract_depletion_width_2d_center` on live 2D device   | Yes — live devsim solve + W extraction | FLOWING (but at restricted voltage range) |
...
| -------------------------------------------- | ----------------------------------------------------------------------------------------------------- | ------------------ | ------ |
| TestCalibrationCV (R² at actual sweep range) | `uv run pytest tests/test_device2d.py::TestCalibrationCV -x -m slow`                                  | PASSED 1 in 16.74s | PASS   |
...
| CONS-01 SC#2 | 26-03-PLAN  | 2D C-V over FULL bias range R² >= 0.99 at center                 | PARTIAL                                | TestCalibrationCV passes but only to -10 V; -15/-20/-30/-40/-50 V not covered     |
...
| -------------------------------------------------------------------------- | ---- | --------------------------------------------------------------------------------- | -------- | --------------------------------------------------------------------------------------------------------------- |
| `tests/test_device2d.py`                                                   | 325  | `voltages = [0.0, -2.5, -5.0, -7.5, -10.0]` in TestCalibrationCV — stops at -10 V | Blocker  | CONS-01 SC#2 full-range assertion not exercised; -15 to -50 V untested by CV criterion                          |
| `.planning/phases/26-graded-doping-2d-calibration/26-REGRESSION-REPORT.md` | 9    | `overall: PASS  # no functional regression...` — inline YAML comment              | Warning  | Plan acceptance criterion `grep -E "^overall: PASS$"` technically fails; verbose but correct verdict documented |
...
One gap blocks CONS-01 SC#2 full closure:

**TestCalibrationCV voltage range truncation.** The plan (26-03-PLAN.md Task 3) and CONS-01 SC#2 require the C-V comparison to cover the full clinical bias range `[0, -5, -10, -15, -20, -30, -40, -50 V]`. The actual test uses `[0.0, -2.5, -5.0, -7.5, -10.0]` — it stops at -10 V and omits -15, -20, -30, -40, -50 V. The calibrated defaults have already been verified to converge at -50 V (TestReverseBiasConvergence), so the solver CAN handle the full sweep; the test simply doesn't exercise it. This is a test-scope gap, not a code bug. The fix is a one-line change to the `voltages` list in `tests/test_device2d.py:325`.

No later phase covers this gap.

**Minor formatting note:** `26-REGRESSION-REPORT.md` appends a YAML comment to the `overall:` key (`overall: PASS  # no functional regression...`). The plan acceptance criterion was `grep -E "^overall: PASS$"` which fails due to the trailing comment. The functional verdict (15/20 PASS, 5 compute-time TIMEOUT, 0 functional regressions) is correctly documented and the rebaseline PI decision is in metadata. This is an informational finding, not a blocker.

---

_Verified: 2026-06-16T09:07:24Z_
