...
nb.cells.append(
    nbformat.v4.new_code_cell(
        ...
        ")\n"
        "from etna.core.charge_collection import (\n"
        "    cce_vs_bias,\n"
...
nb.cells.append(
    nbformat.v4.new_markdown_cell(
        ...
        "reverse bias (-3V).\n"
        "\n"
        "**Why -3V?** At this bias the depletion width (~5.3 um for N_D_bulk = 8.5e13) "
        "is smaller than the thicker epi layers but comparable to the thinnest. This "
        "creates the partial depletion regime where the epi thickness effect is visible.\n"
        "\n"
        "**Expected physics:**\n"
        ...
        "- For epi > alpha range (~15 um), CCE may increase as the generation profile "
        "shape changes (less concentrated near the surface)\n"
...
nb.cells.append(
    nbformat.v4.new_markdown_cell(
        ...
        "3. **Thinner epi gives higher CCE at low bias (-3V)** demonstrating the "
        "partial depletion trade-off. Non-monotonic behavior for epi > alpha range "
        "reflects the generation profile shape changing. At high bias (-30V) all "
        "thicknesses are fully depleted and CCE ~ 1.0.\n"
