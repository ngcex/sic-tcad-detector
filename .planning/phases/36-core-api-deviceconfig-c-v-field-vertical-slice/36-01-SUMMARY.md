...
requires:
  - phase: 35-package-setup-refactor
    provides: installable etna package with etna/core/ (renamed from src/)
provides:
  - etna/api/ package (device.py, results.py, __init__.py)
  ...
  - etna/__init__.py re-exporting DeviceConfig, SimResult, MeshData, __version__
...
tech-stack:
  added: []
  patterns:
    - "etna/api/ facade layer over etna/core/ internal modules — public contract only imports absolute etna.api.*/etna.core.* paths, never top-level etna, to avoid import cycles"
...
- Established `etna/api/` package as the canonical home for the public API contract
- `SimResult` and `MeshData` dataclasses match design spec sections 3.2/3.3 exactly (field names and order)
- `DeviceConfig` promoted out of the `etna/__init__.py` stub into `etna/api/device.py`, all 11 fields with spec-correct defaults preserved verbatim
...
- `etna/__init__.py` now the single public entry point re-exporting `DeviceConfig`, `SimResult`, `MeshData`, `__version__`, with no duplicate dataclass definitions

## Task Commits

1. **Task 1: Create SimResult and MeshData dataclasses in etna/api/results.py** - `111973e` (feat)
2. **Task 2: Promote DeviceConfig to etna/api/device.py and add build_device() facade** - `fa6d586` (feat)
3. **Task 3: Re-export public API from `etna/__init__.py` and remove the stub** - `d68460f` (refactor)

_Note: no TDD tasks in this plan; each task is a single commit._

## Files Created/Modified

- `etna/api/__init__.py` - package marker with one-line docstring
- `etna/api/results.py` - `MeshData` and `SimResult` dataclasses, TYPE_CHECKING-only reference to `DeviceConfig`
- `etna/api/device.py` - canonical `DeviceConfig` dataclass + `build_device(config, device_name=None)` facade
- `etna/__init__.py` - rewritten to re-export the public API; inline `DeviceConfig` stub removed
...
- `results.py` uses `TYPE_CHECKING`-only import of `DeviceConfig` from `etna.api.device` to type the `SimResult.config` field without creating a runtime import cycle (results.py loads before device.py in `etna/__init__.py`, and device.py never imports results.py).
...
- `DeviceConfig`, `SimResult`, `MeshData` are importable from `etna` with the exact contract Plans 02 (`run_cv`) and 03 (`run_field`) depend on.
...
- `etna/__init__.py` has a marked extension point (`# run_cv, run_field added by Phase 36 Plans 02/03`) for the next two plans to append their re-exports.
- Existing test suite unaffected: `pytest tests/test_cv.py -q` — 13 passed.
- No blockers for Plans 02/03.

## Self-Check: PASSED

- FOUND: `etna/api/__init__.py`
- FOUND: `etna/api/results.py`
- FOUND: `etna/api/device.py`
