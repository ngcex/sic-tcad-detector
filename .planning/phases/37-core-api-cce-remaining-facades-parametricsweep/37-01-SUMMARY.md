...
provides:
  ...
  - "run_cce re-exported from etna (from etna import run_cce)"
...
- `run_cce()` added to `etna/api/simulation.py`: 2D guard, `np.linspace` bias sweep, single `cce_vs_bias` call with config-forwarded `device_kwargs`, `SimResult` repackaging with `I_collected`/`I_generated` metadata.
- Re-exported `run_cce` from `etna/__init__.py` (import + `__all__`).
...
- `etna/api/simulation.py` - Added `run_cce()` facade + `from etna.core.charge_collection import cce_vs_bias` import.
- `etna/__init__.py` - Re-export `run_cce` (import line + `__all__`).
...
- **Issue:** The worktree has no `.venv`; the main checkout's editable `etna` install (`_editable_impl_etna.pth`) points at the main checkout root. The plan's literal verify command `.venv/bin/pytest` uses the bare console script, which does NOT put the worktree root on `sys.path[0]` — so `import etna` would resolve to the main checkout, not the worktree, silently testing the wrong source.
- **Fix:** Ran tests as `<main>/.venv/bin/python -m pytest ...` from the worktree root (`python -m pytest` prepends cwd to `sys.path`, so the worktree `etna/` wins). Verified before RED and after GREEN via a path guard: `python -c "import etna; print(etna.__file__)"` confirmed resolution to `agent-ab46ba69c3408ae97/etna`. All physics/behavior assertions in the plan are unchanged; only the harness invocation differs.
...
- LIB-04 satisfied: `from etna import run_cce`; `run_cce(DeviceConfig())` returns `SimResult(sim_type="cce")` with CCE in [0, 1].
...
- `etna/api/simulation.py::run_cce` — FOUND (grep -c "def run_cce" == 1)
- `etna/__init__.py` run_cce re-export — FOUND
