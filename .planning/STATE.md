...
- `etna/core/` deliberately not created in Plan 35-01 — reserved for Plan 35-02's `git mv src etna/core` to avoid nesting bug
...
- Plan 35-02 complete: `src/` renamed to `etna/core/`, all 326 imports + 81 notebook imports + 6 string-literal refs rewritten, zero residual `from src.` confirmed — see `35-02-SUMMARY.md`
- `Path(__file__).parent` chains must be re-verified whenever a module moves to a different directory depth — `microdosimetry.py`'s `data_dir` resolution needed one more `.parent` after the `src/` -> `etna/core/` rename added a nesting level (fixed in 35-02)
...
- **Phase 39 page import structure (RESEARCH A6, confirmed by 39-01 spike):** `etna.run_*` facades must be referenced as module attributes (`import etna; etna.run_cv(cfg)`) in page code, never `from etna import run_cv` — empirically proven mockable via `monkeypatch.setattr(etna, "run_cv", fake)` under `AppTest.from_function` in `tests/test_app_run_mockability.py`. Mandatory for 39-03/39-04 page implementations.
- **Phase 39-02 CCE CSV schema (confirmed by source read, not devsim run):** `cce_vs_bias`'s `I_collected` is a bias-aligned numpy array (`I_sorted`) -> CSV column `I_collected_A_per_cm2`; `I_generated` is a scalar total generated current (`Q*np.trapezoid(...)`) -> `# I_generated_A_per_cm2:` metadata header line, never a broadcast column. Ground truth for all future CCE CSV/export work — see `etna/core/charge_collection.py` `cce_vs_bias` return block and `39-02-SUMMARY.md`.
...
- Plan 39-04 complete: field map page mirrors the sibling 39-03 page/test structure (`etna.run_field` referenced as a module attribute for monkeypatch mockability); 1D-only pre-check placed BEFORE `run_field` since it silently returns empty x/y arrays for 2D configs instead of raising — see `39-04-SUMMARY.md`.
- Plan 39-03 complete: C-V and CCE pages mirror the 39-04 field_map page/test structure exactly (`etna.run_cv` / `etna.run_cce` referenced as module attributes for monkeypatch mockability); both use a 1D-only pre-check before the facade call since `run_cv`/`run_cce` raise `NotImplementedError` for 2D configs (unlike `run_field`, which silently returns empty arrays) — see `39-03-SUMMARY.md`.
...
- `src/` flat layout with no stable public API → `etna/core/` + `etna/api/`
- `requirements.txt` → `pyproject.toml` with proper package metadata
- Notebooks import internal classes directly → all imports via `etna.*` public API
