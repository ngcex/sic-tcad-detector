...
**Issue:** Manipulating `sys.path` at module import time to make `app.*` importable is a common Streamlit pattern but is a code smell — it's fragile to being run from unexpected working directories and makes the module's importability implicit rather than explicit (e.g. via a proper editable install / `pyproject.toml` package config, which this project already seems to use given `petringa` is importable elsewhere without such a shim).
**Fix:** If `petringa` and `app` are both installed in editable mode (`uv pip install -e .`) with `app` declared as a package, this shim becomes unnecessary. Low priority — functions correctly today.
